...
def _direct_linear_solve(model, base_x, y_base, y_target, weights, 
    ...
    # 计算敏感度（雅可比的绝对值）
    sensitivity = np.abs(jacobian)
    sensitivity = np.maximum(sensitivity, 1e-6)
    
    # 按敏感度和权重分配变化量
    # Δx_i = weight_i * delta_y / sensitivity_i
    delta_x = (weights * delta_y) / sensitivity
    
    # 限制变化幅度（防止过度调整）
    max_change = np.maximum(0.5 * base_x, 1.0)
    delta_x = np.clip(delta_x, -max_change, max_change)
    
    # 应用约束
    suggested = base_x + delta_x
    suggested = _apply_constraints(suggested, base_x, min_constraints, 
